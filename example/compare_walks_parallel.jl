...
@everywhere function print_header() 
  println("Length of Paths to Optima and Fitness of Optima for random, greedy, and reluctant walks")
  ...
  println("ln:   length")
  println("ft:   fitness")
  println("N\tK\tq\tμ rndln\tσ rndln\tμ rndft\tσ rndft\tμ grdln\tσ grdln\tμ grdft\tσ grdft\tμ rltln\tσ rltln\tμ rltft\tσ rltft")
end
...
@everywhere function print_results(a::AllResults) 
    ...
    @printf("\t%.3f\t%.3f\t%.3f\t%.3f\n", mean(a.reluct_len), std(a.reluct_len), mean(a.reluct_fit), std(a.reluct_fit))
end
