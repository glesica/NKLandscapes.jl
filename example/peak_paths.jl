...
References:  Gavrilets, Sergey, and Janko Gravner.  1997. doi: 10.1006/jtbi.1996.0242.
Gravner, Janko, Damien Pitman, and Sergey Gavrilets. 2007.  doi: 10.1016/j.jtbi.2007.07.009.

TODO:  This introductory summary could be improved.

To test the functions in this file, do:
fitdiff_weight = 8.0   # Weight on fitness differences as opposed to the number of mutational steps.
ls = NKLandscape(5,3)
fits = lsfits(ls)
test_paths(ls,fits,fitdiff_weight)

To print a summary of the paths between peaks for N=8, K=2 NK Landscapes, do:
fitdiff_weight = 8.0   # Weight on fitness differences as opposed to the number of mutational steps.
n = 8  # N for the NK landscape
k = 2  # K for the NK landscape
num_landscapes = 5
print_paths_summary(n, k, num_landscapes, fitdiff_weight)
=#
...
"Least cost" is defined by the function "edge_cost_funct" which uses the parameter "fit_diff_weight" to control the relative
contribution of path length and fitness decrease.
...
function least_cost_paths(source::Genotype, destinations::Set{Genotype}, ls::NKLandscape, fits::Vector{Float64},
    fit_diff_weight::Float64; edge_cost_funct=edge_cost_exp, single_path=false)
  all_paths = Path[]
  ...
  while !isempty(queue)
    queue_node = Base.Collections.dequeue!(queue)
    if queue_node.current in destination_alleles
      setdiff!(destination_alleles, queue_node.current)
      len = length(queue_node.previous)
      path = zeros(AlleleString,len)
      min_fitness = 1.0  # maximum possible fitness
      i = len
      for g in queue_node.previous
        fg = fits[g+1]
      ...
      push!(all_paths,Path(path,queue_node.cost,decrease_fitness))
      if single_path
        return all_paths
      end
    end
    if !in(queue_node.current, closed)
      ...
      for nbr = neighbors(Genotype(queue_node.current,ls))
        ...
        if (value == Void) || (value < new_cost)
          nbr_node = QueueNode(nbr.alleles,cons(nbr.alleles,queue_node.previous), new_cost + queue_node.cost)
          queue[nbr_node] = new_cost + queue_node.cost
        end
      end
    end
  end
...
"""
function path_cost(path::Array{AlleleString,1},ls::NKLandscape, fit_diff_weight::Float64)
  cost = 0.0
  gprev = path[1]
...
@doc """ function summary_landscape_paths(n::Int64, k::Int64; peak_dist_cutoff::Int64=1000)
...
is less than or equal to "peak_dist_cutoff".
Returns a LandscapePathSummary object which summarizes the statistics of these paths.
"""
function summary_landscape_paths(n::Int64, k::Int64, fit_diff_weight::Float64; peak_dist_cutoff::Int64=1000)
  if k == 0
  ...
  for i = 1:length(bcc)
    gi = bcc[i].gtype
    setdiff!(g_set,[gi])
    filtered_set = filter(x->hamming_dist(x,gi)<peak_dist_cutoff, g_set)
    spw = least_cost_paths(gi, filtered_set, ls, fts, fit_diff_weight)
...
Processing of landscapes is distributed across processes by using pmap, 
   so this function will run faster with multiple processes.
"""
function print_paths_summary(n, k, num_landscapes, fit_diff_weight::Float64; peak_dist_cutoff::Int64=1000)
  @assert num_landscapes > 1
  println("fit diff weight: ",fit_diff_weight)
  println("peak_dist_cutoff: ",peak_dist_cutoff)
  ...
  path_summary_list = 
      pmap(n_k->summary_landscape_paths(n_k[1], n_k[2], fit_diff_weight, peak_dist_cutoff=peak_dist_cutoff), [(n,k) for _ = 1:num_landscapes])
  ...
  if length(bcc) > 1
    for i = 1:length(bcc)
      ...
      for j = i+1:length(bcc)
        ...
        spw_ij = least_cost_paths(gi,gj_set,ls,fits,fit_diff_weight,single_path=true,edge_cost_funct=edge_cost_sym)
        spw_ji = least_cost_paths(gj,gi_set,ls,fits,fit_diff_weight,single_path=true,edge_cost_funct=edge_cost_sym)
